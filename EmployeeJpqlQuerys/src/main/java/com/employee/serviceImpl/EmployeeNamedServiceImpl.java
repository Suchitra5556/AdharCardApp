...
public class EmployeeNamedServiceImpl implements EmployeeNamedService {
	...
	@Override
	public List<Employee> getByAddressAndNameUnOrder(String address, String name) throws EmployeeNotFoundWithAddressAndNameException {
		
		List<Employee> byAddressAndNameUnOrder = employeeNamedRepo.findByAddressAndNameUnOrder(address, name);
		 System.out.println("Exceptio working flow"+ byAddressAndNameUnOrder);
		if(byAddressAndNameUnOrder==null || byAddressAndNameUnOrder.size()<=0)
